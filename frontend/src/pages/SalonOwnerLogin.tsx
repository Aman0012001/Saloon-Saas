  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 selection:bg-accent/30">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        {/* Login Node */}
        <Card className="w-full max-w-xl border-none shadow-[0_40px_80px_-20px_rgba(0,0,0,0.08)] bg-slate-900/90 backdrop-blur-3xl rounded-[3rem] overflow-hidden border border-white/10">
          <CardHeader className="text-center pt-16 pb-8 relative">
            ...
            {!authLoading && user && !(user.user_type === 'salon_owner' || user.salon_role) && (
              <div className="mb-8 p-4 bg-accent/10 border border-accent/20 rounded-2xl animate-fade-in">
                <p className="text-accent text-sm font-bold mb-3 flex items-center justify-center gap-2">
                ...
                <Button
                  ...
                  className="h-10 bg-white/5 border-white/10 text-white hover:bg-white/10 rounded-xl font-bold px-6"
                >
            ...
            {!authLoading && user && (user.salon_role === 'staff' || user.salon_role === 'manager') && (
              <div className="mb-8 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl animate-fade-in">
                ...
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-3 text-center">Your terminal session is ready.</p>
                <Button
            ...
            <div className="w-20 h-20 bg-accent rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-accent/40 relative z-10">
              <Store className="w-10 h-10 text-black" />
            </div>
            <CardTitle className="text-4xl font-black text-white tracking-tight relative z-10">Station Login</CardTitle>
            <CardDescription className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 relative z-10">Authenticated Management Access</CardDescription>
          </CardHeader>
